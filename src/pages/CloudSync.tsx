              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 mt-3 sm:mt-4">
                    ...
                    <button
                      onClick={async () => {
                        await db.cloudAccounts.update(account.id, { autoSync: !account.autoSync });
                      }}
                      className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${
                        account.autoSync ? 'bg-[var(--accent-mint)]' : 'bg-[var(--glass-border)]'
                      }`}
                      title={account.autoSync ? '关闭自动同步' : '开启自动同步'}
                    >
                      <motion.div
                        animate={{ x: account.autoSync ? 22 : 2 }}
